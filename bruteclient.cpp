...
void brute(int start, int end) {
    ...
    for (int a = end ; a > start; a-=2) {
        key[0] = char(a);
        for (int b = 126; b > 33; b-=2) {
            key[1] = char(b);
            for (int c = 126; c > 33; c-=2)  {
                key[2] = char(c);
                for (int d = 126; d > 33; d-=2) {
                    key[3] = char(d);
                    for (int e = 126; e > 33; e-=2) {
                        key[4] = char(e);
                        for (int f = 126; f > 33; f-=2) {
                            key[5] = char(f);
                            for (int g = 126; g > 34; g-=2) {
                                key[6] = char(g);
